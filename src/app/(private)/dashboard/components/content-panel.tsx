...
const formatRelativeDate = (date: Date): string => {
  ...
  if (recordingDay.getTime() === today.getTime()) {
    ...
    return recordingDate.toLocaleDateString("pt-BR", {
      day: "2-digit",
      month: "short",
    });
  }
...
export function ContentPanel({ className }: ContentPanelProps) {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredRecordings.length / CARDS_PER_VIEW),
  );
  ...
  return (
    <motion.div
      ...
      className={cn(
        "flex h-full flex-col rounded-2xl border border-gray-100 bg-white p-4 shadow-sm lg:p-5",
        className,
      )}
    >
      {/* Header */}
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <div className="relative max-w-xs flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-gray-100 bg-gray-50 py-1.5 pr-3 pl-8 text-xs text-gray-700 transition-all outline-none placeholder:text-gray-400 focus:border-sky-200 focus:bg-white focus:ring-1 focus:ring-sky-200"
          />
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              onClick={nextSlide}
              disabled={
                currentIndex === maxIndex || filteredRecordings.length === 0
              }
              className="flex h-7 w-7 items-center justify-center rounded-lg border border-gray-100 bg-white text-gray-400 transition-all hover:border-sky-200 hover:text-sky-600 disabled:opacity-30 disabled:hover:border-gray-100 disabled:hover:text-gray-400"
            >
            ...
            {isGettingRecordings ? (
              ...
              currentRecordings.map((recording) => (
                <div
                  key={recording.id}
                  onClick={() =>
                    handleRecordingClick(recording.id, recording.type)
                  }
                ...
                >
                  ...
                  <div className="mb-3 flex items-start justify-between gap-3">
                    ...
                    <div className="flex-1 text-right">
                      <span className="inline-block rounded-md bg-sky-50 px-2 py-0.5 text-[10px] font-bold tracking-wide text-sky-700 uppercase">
                        {recording.type}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="mt-auto flex items-center justify-between border-t border-gray-100 pt-2">
                    <div className="flex gap-1">
                      {recording.tags.slice(0, 2).map((tag) => (
                        <span
                          key={tag}
                          className="inline-block max-w-[70px] truncate rounded bg-gray-100 px-1.5 py-0.5 text-[9px] text-gray-600"
                        >
                          {tag}
                        </span>
                      ))}
